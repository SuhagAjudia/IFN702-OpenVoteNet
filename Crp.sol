...
library Secp256k1_noconflict {
    ...
    function validateSignature(bytes32 message, uint[2] rs, uint[2] Q) internal pure returns (bool) {
        uint n = nn;
        uint p = pp;
    ...
    function decompress(uint8 yBit, uint x) internal pure returns (uint[2] P) {
        uint p = pp;
...
contract LocalCrypto {
  ...
  function commitToVote(uint[4] params, uint[2] xG, uint[2] yG, uint[2] y, uint[2] a1, uint[2] b1, uint[2] a2, uint[2] b2) public view returns (bytes32) {
  ...
  // r = v - xz (mod p);
  // return(r,vG)
  function createZKP(uint x, uint v, uint[2] xG) public returns (uint[4] res) {
  ...
  function verifyZKP(uint[2] xG, uint r, uint[3] vG) public returns (bool){
      //uint[2] memory G;
      G[0] = Gx;
      G[1] = Gy;
  ...
  // random 'w', 'r1', 'd1'
  function create1outof2ZKPNoVote(uint[2] xG, uint[2] yG, uint w, uint r2, uint d2, uint x) public view returns (uint[10] res, uint[4] res2){
      ...
      bytes32 b_c = keccak256(abi.encodePacked(msg.sender, xG, res));
  ...
  // random 'w', 'r1', 'd1'
  // TODO: Make constant
  function create1outof2ZKPYesVote(uint[2] xG, uint[2] yG, uint w, uint r1, uint d1, uint x) public view returns (uint[10] res, uint[4] res2) {
      // y = h^{x} * g
      uint[3] memory temp1 = Secp256k1_noconflict._mul(x,yG);
  ...
  function verify1outof2ZKP(uint[4] params, uint[2] xG, uint[2] yG, uint[2] y, uint[2] a1, uint[2] b1, uint[2] a2, uint[2] b2) public view returns (bool) {
      uint[2] memory temp1;
      uint[3] memory temp2;
      uint[3] memory temp3;
    ...
    function createCommitment(uint r, uint b) public view returns (uint[2]){
    ...
    function openCommitment(uint[2] c, uint r, uint b) public view returns (bool) {
    ...
    // return t,n.
    function createEqualityProof(uint r1, uint r2, uint r3, uint[2] c1, uint[2] c2) public view returns (uint[2] t, uint n) {
      ...
      // TODO: add msg.sender
      uint h = uint(keccak256(abi.encodePacked(msg.sender, G, Y, c1, c2, t)));
    ...
    function computeFirstHalfEquality(uint[2] c1, uint[2] c2, uint h, uint[2] t) public pure returns (uint[2] left){
    ...
    // 2. Does nY == h*(c1-c2) + t
    function verifyEqualityProof(uint n,  uint[2] c1, uint[2] c2, uint[2] t) public view returns (bool) {
      if(!Secp256k1_noconflict.isPubKey(c1)) { revert(); }
      if(!Secp256k1_noconflict.isPubKey(c2)) { revert(); }
      if(!Secp256k1_noconflict.isPubKey(t)) { revert(); }
    ...
    function createInequalityProof(uint b1, uint b2, uint r1, uint r2, uint r3, uint r4, uint[2] c1, uint[2] c2) public view returns (uint[2] t1, uint[2] t2, uint n1, uint n2) {
      ...
      // TODO: add msg.sender
      uint h = uint(keccak256(abi.encodePacked(msg.sender, G, Y, c1, c2, t1, t2)));
    ...
    function computeSecondHalfInequality(uint[2] c1, uint[2] c2, uint[2] t2, uint h) public pure returns (uint[3] right) {
      uint[3] memory negative_c2;
      // Negate the 'y' co-ordinate of C2
      negative_c2[0] = c2[0];
      negative_c2[1] = pp - c2[1];
    ...
    function verifyInequalityProof(uint[2] c1, uint[2] c2, uint[2] t1, uint[2] t2, uint n1, uint n2) public view returns (bool) {
      if(!Secp256k1_noconflict.isPubKey(c1)) { revert(); }
      if(!Secp256k1_noconflict.isPubKey(c2)) { revert(); }
      if(!Secp256k1_noconflict.isPubKey(t1)) { revert(); }
      if(!Secp256k1_noconflict.isPubKey(t2)) { revert(); }
